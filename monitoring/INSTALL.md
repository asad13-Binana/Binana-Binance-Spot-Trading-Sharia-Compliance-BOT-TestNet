...
six bot containers pass their health gate. It creates the `botmon` system user,
...
```bash
sudo grep '^MONITOR_TOKEN=' /etc/binana-freqtrade-v101/testnet-monitor.env
...
```bash
sudo systemctl enable --now binana-monitor-report-testnet.timer
```

For live, edit `/etc/binana-freqtrade-v101/live-monitor.env` only after the
...
Never copy `/etc/binana-freqtrade-v101/.env` into the monitor service. The
